...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="JSON config for the Pkzz relay/Postgres provisioner",
    )
    parser.add_argument(
        "--buzz-bin-dir",
        type=Path,
...
def write_metadata_template(args: argparse.Namespace, job_dir: Path) -> Path:
    ...
    metadata = {
        "agent_url": "https://github.com/kingkillery/pkzz",
        "agent_display_name": f"Pkzz Orchestra ({manifest.get('condition', 'team')})",

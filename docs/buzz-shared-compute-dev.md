# Pkzz shared compute: local GUI verification

This runbook verifies the actual desktop path used by the built-in **Fizz** agent:

`Pkzz Desktop → buzz-acp → buzz-agent → MeshLLM SDK → local/remote compute`

It does not use a substitute agent harness.

## Before starting

Run from the `kingkillery/pkzz` repository root on the mesh-enabled branch.
...
`~/.buzz-dev`, and local Docker volumes; it preserves the installed Pkzz app's
...
If using `mesh-dev-fresh`, the clean window opens at **Welcome to Pkzz**. Join
the seeded local community before continuing:

1. Click **Join a community**.
2. Use any local name, such as **Local Pkzz**.
...
Stop only stale Pkzz/MeshLLM processes shown by that command. Do not leave a
...
take several minutes. Wait for the Pkzz window to open and for the terminal to
...
Pkzz may download the model on first use. The model picker ranks models for the
...
**buzz-agent** as the runtime, **Pkzz shared compute** as the LLM provider,
...
- no current Pkzz membership snapshot is available (admission fails closed).

## Security boundary

Pkzz publishes member-signed discovery notes through an ordinary relay-supported
...
tokens in the note. Current Pkzz membership controls which node identities are
...
MeshLLM—not the Pkzz relay—carries inference over direct QUIC or its encrypted
...
is an upstream protocol change and is not claimed by the Pkzz-side checks above.

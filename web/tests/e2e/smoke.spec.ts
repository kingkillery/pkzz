...
test("home page loads with Pkzz branding", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("main").getByRole("img", { name: "Pkzz" }),
  ).toBeVisible();
});
...
test("invite requires age and legal consent before opening Pkzz", async ({
  page,
}) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify([
        { draft: false, prerelease: false, assets: [] },
        {
          ...
          assets: [
            {
              name: "Pkzz_0.4.9_aarch64.dmg",
              browser_download_url:
                "https://github.com/kingkillery/pkzz/releases/download/v0.4.9/Pkzz_0.4.9_aarch64.dmg",
            },
            {
              name: "Pkzz_0.4.9_x64.dmg",
              browser_download_url:
                "https://github.com/kingkillery/pkzz/releases/download/v0.4.9/Pkzz_0.4.9_x64.dmg",
            },
            {
              name: "Pkzz_0.4.9_amd64.AppImage",
              browser_download_url:
                "https://github.com/kingkillery/pkzz/releases/download/v0.4.9/Pkzz_0.4.9_amd64.AppImage",
            },
            {
              name: "Pkzz_0.4.9_x64-setup_alpha-unsigned.exe",
              browser_download_url:
                "https://github.com/kingkillery/pkzz/releases/download/v0.4.9/Pkzz_0.4.9_x64-setup_alpha-unsigned.exe",
            },
          ],
        },
      ]),
  ...
  const agreementConfirmation = page.getByLabel(
    "I agree to the Pkzz Terms of Service and Privacy Policy.",
  );
  const acceptInvite = page.getByRole("button", {
    name: "Accept invite in Pkzz",
  });
  ...
  await page
    .locator("label")
    .filter({
      hasText: "I agree to the Pkzz Terms of Service and Privacy Policy.",
    })
...
test("invite asks Safari users to choose their Mac download", async ({
  ...
  await expect(openedPage).toHaveURL("https://github.com/kingkillery/pkzz/releases");
...
test("invite download falls back for mobile and non-desktop devices", async ({
  ...
  for (const device of unsupportedDevices) {
    ...
    await expect(
      page.getByRole("link", { name: "Download it now" }),
      device.name,
    ).toHaveAttribute("href", "https://github.com/kingkillery/pkzz/releases");
    await context.close();
  }
});

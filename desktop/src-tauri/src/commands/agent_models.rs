...
pub async fn discover_agent_models(
    ...
    // Pkzz shared compute discovery must not depend on the local OpenAI ingress: that
        ...
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        ...
        .await
        .map_err(|error| format!("Pkzz shared compute model discovery failed: {error}"))?;
        ...
        if availability.models.is_empty() {
            return Err(availability.reason.unwrap_or_else(|| {
                "No live Pkzz shared compute models are available".to_string()
            }));
        }
    ...
    if input.provider.as_deref().map(str::trim)
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        return Err("Pkzz shared compute is not available in this build".to_string());
    }

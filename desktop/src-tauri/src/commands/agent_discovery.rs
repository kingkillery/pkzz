...
fn install_acp_runtime_blocking(
    ...
    // Pkzz-private prefix before execution.
...
/// costs Pkzz's managed Node/npm dirs their precedence. A positional rather than
...
/// Pkzz-managed npm locations set, and the user's PATH set. This is the
...
fn install_shell_command(command: &str) -> Result<std::process::Command, String> {
    ...
    // where the composed PATH would otherwise be Pkzz's managed Node dirs
...
/// Strips hermit-managed vars and establishes the Pkzz-managed npm prefix so adapters
...
fn install_powershell_command(command: &str) -> std::process::Command {
    ...
    // Compose PATH: managed Pkzz dirs first, then inherited process PATH.
...
mod tests {
    ...
    fn test_install_shell_command_includes_process_path_on_windows() {
        ...
        // The sentinel must appear LAST — managed Pkzz dirs must have precedence.
        assert!(

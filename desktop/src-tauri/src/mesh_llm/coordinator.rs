...
//!
//! Pkzz publishes a client-signed, replaceable discovery note containing the
...
//! The Pkzz relay is only a generic Nostr store for membership and discovery;
...
/// NIP-51 bookmark-set kind with a reserved d-tag so existing Pkzz relays accept
...
/// the relay. Recheck promptly so simultaneous starts converge into one Pkzz
...
/// Join an isolated runtime to the existing Pkzz community mesh. The relay is
...
enum RosterReconcileAction {
    ...
    Keep,
    /// Restart Pkzz so MeshLLM is rebuilt with a freshly resolved roster.
    ///
    /// MeshLLM's native listeners are process-owned in practice: stopping and
    /// starting the embedded runtime in one process can terminate Pkzz or race
...
async fn reconcile_roster(
    ...
    drop(guard);
    eprintln!(
        "buzz-mesh: membership roster changed; restarting Pkzz to rebuild MeshLLM with the fresh community allowlist"
    );
    app.request_restart();
    Ok(())
}

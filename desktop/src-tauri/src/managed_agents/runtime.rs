...
pub fn spawn_agent_child(
    ...
    // ── Git credential helper for Pkzz relay ──────────────────────────
    //
    // Agents need to clone/push repos hosted on the Pkzz relay's git
    ...
    if let Some(cred_helper) = resolve_command("git-credential-nostr") {
        ...
        eprintln!(
            "buzz-desktop: git-credential-nostr not found — agent {} will not have automatic Pkzz git auth",
            record.name,
        );
    }
    ...
    // applied. Writing it last lets user-provided values win over every Pkzz-set env
    ...
    // Pkzz shared compute is stored as a native provider; derive the OpenAI-compatible

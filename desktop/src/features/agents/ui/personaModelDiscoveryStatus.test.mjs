...
test("Pkzz shared compute names the empty state and next action", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("no Pkzz shared compute serving members are available"),
    "relay-mesh",
  );
...
test("Pkzz shared compute names a missing relay member roster", () => {
  const status = formatModelDiscoveryErrorStatus(
    new Error("Pkzz shared compute is waiting for the current member roster"),
    "relay-mesh",
  );

...
test("tab actions restore terminal input focus", async () => {
  ...
  const actions = [
    ...
    ["new", view.getByLabelText("New Pkzz Term tab")],
  ];
...
test("drag resize batches visual updates and commits state only on release", async () => {
  ...
  await ready(view);
  const substrate = view.container.querySelector(".buzz-terminal-substrate");
  const handle = view.getByLabelText("Resize Pkzz Term");
...
test("a hidden dock supplies an inline zero height", async () => {
  ...
  assert.notEqual(substrate.style.height, "0px");
...
test("canvas failure atomically restores Pkzz ownership", async () => {
...
test("cursor blink runs only while the terminal owns input and resets on input", async () => {
  ...
  try {
    ...
    await ready(subject.view);
    assert.equal(callbacks.size, 0, "Pkzz ownership must pause blinking");
...
test("tab chords stay inert while Pkzz owns input", async () => {
  ...
  assert.equal(
    ...
    false,
    "Pkzz-mode ⌘T belongs to the rest of the app",
  );
});

...
test.describe("observer feed screenshots", () => {
  ...
  test("11 — first-turn bundle: standalone system-prompt card + Checks ingress (per-turn context only)", async ({
    ...
    // (Pkzz event / Thread context) — no Base/System/Team Instructions/Core Memory/Channel Canvas sections.
    await seedObserverEvents(page, OBSERVER_AGENT_PUBKEY, [
      {
        seq: 1,
        timestamp: NOW,
    ...
    // Per-turn prompt context (Pkzz event / Thread context) does NOT appear
    ...
    // (Pkzz event, Thread context). Base/System/Team Instructions/Core Memory/Channel Canvas must NOT appear.
    ...
    // Only per-turn context sections (Pkzz event + Thread context) — no system-prompt sections.
    expect(sectionTitles.length).toBe(2);
    expect(sectionTitles[0]).toContain("Pkzz event");

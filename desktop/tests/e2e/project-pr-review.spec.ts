...
test("repository rows identify their git host", async ({ page }) => {
  ...
  await expect(
    page.getByRole("tooltip", { name: "Pkzz-hosted repository" }),
  ).toBeVisible();
});
...
test("project without a checkout offers fetch feedback and dropdown cloning", async ({
  ...
  await expect(
    page.getByRole("button", { name: "Pkzz", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Pkzz", exact: true }),
  ...
  await page.getByRole("button", { name: "Pkzz", exact: true }).click();

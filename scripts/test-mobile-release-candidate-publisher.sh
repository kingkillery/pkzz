...
case "${1:-}:${2:-}" in
  api:repos/kingkillery/pkzz/rulesets/14378754)
    case "$*" in
    ...
    ;;
  api:repos/kingkillery/pkzz/git/ref/heads/main) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:repos/kingkillery/pkzz/commits/*) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:--paginate)
    [[ "$3" == "repos/kingkillery/pkzz/git/matching-refs/tags/mobile-v1.2.3-rc." ]]
    printf '%s' "${GH_EXISTING_REFS:-}"
    ;;
  api:--method)
    endpoint="$4"
    case "$endpoint" in
      repos/kingkillery/pkzz/git/tags)
        record "$*"
        printf '%s\n' "$GH_TAG_OBJECT_SHA"
        ;;
      repos/kingkillery/pkzz/git/refs)
        record "$*"
        ;;
      *) exit 2 ;;
    esac
    ;;
  api:repos/kingkillery/pkzz/git/ref/tags/mobile-v1.2.3-rc.*)
    if [[ "$*" == *'.object.type'* ]]; then
    ...
    ;;
  api:repos/kingkillery/pkzz/git/tags/*)
    if [[ "$*" == *'.object.type'* ]]; then
...
export GITHUB_REPOSITORY=kingkillery/pkzz
...
grep -Fq -- '-f message=Pkzz Mobile 1.2.3 release candidate 1' "$GH_CALLS"

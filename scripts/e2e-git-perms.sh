...
# =============================================================================
# Two bots collaborate on a simple web page via the Pkzz relay's git server.
#
# Prerequisites:
...
for i in $(seq 1 "$RELAY_STARTUP_TIMEOUT"); do
    if curl -sf --max-time 2 "${RELAY_HTTP}/" -H "Accept: application/nostr+json" | grep -q "Pkzz"; then
        break
    fi
...
<html>
<head>
    <title>Pkzz E2E Test Page</title>
    <style>
...
<body>
    <h1>🐝 Pkzz Collaborative Page</h1>
    <p>This page was created by two bots collaborating via Pkzz's git server.</p>
...
sed -i.bak '/<\/body>/i\
    ...
    <footer>\
        <p><em>Built with Pkzz sovereign git hosting</em></p>\

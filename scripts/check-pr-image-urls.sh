...
# Pkzz/relay media URLs often render in Pkzz but fail in GitHub PR markdown
...
if [[ -s "$tmp_matches" ]]; then
  ...
  echo "error: PR markdown contains Pkzz/relay media URLs that may not render on GitHub:" >&2
  printf '%s\n' "$matches" >&2
  echo >&2

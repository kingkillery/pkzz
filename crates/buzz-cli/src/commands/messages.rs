...
pub struct SendMessageParams {
    ...
    pub ompk_execution: bool,
    pub ompk_cwd: Option<String>,
}

const OMPK_EXECUTION_TAG: &str = "ompk-execution";
const OMPK_EXECUTION_CONTRACT_VERSION: &str = "1";
const OMPK_CWD_TAG: &str = "ompk-cwd";
const MAX_OMPK_CWD_BYTES: usize = 4096;

// The versioned execution tag is the OMPK adapter envelope discriminator.
// Placement fields are separate structural tags so OMPK can add supported
// constraints later without turning buzz-cli into a runner or scheduler.

#[derive(Debug, PartialEq, Eq)]
struct OmpkExecutionMetadata {
    cwd: Option<String>,
}

fn validate_ompk_execution_metadata(
    execution: bool,
    cwd: Option<&str>,
) -> Result<Option<OmpkExecutionMetadata>, CliError> {
    if !execution {
        return if cwd.is_some() {
            Err(CliError::Usage(
                "--ompk-cwd requires --ompk-execution".into(),
            ))
        } else {
            Ok(None)
        };
    ...
    if let Some(cwd) = cwd {
        if cwd.is_empty()
            || cwd.len() > MAX_OMPK_CWD_BYTES
            || cwd.chars().any(char::is_control)
            || !Path::new(cwd).is_absolute()
        {
            return Err(CliError::Usage(
                "--ompk-cwd must be a non-empty absolute path without control characters".into(),
            ));
        }
    }

    Ok(Some(OmpkExecutionMetadata {
        cwd: cwd.map(str::to_owned),
    }))
}

fn with_ompk_execution_metadata(
    builder: EventBuilder,
    metadata: Option<&OmpkExecutionMetadata>,
) -> Result<EventBuilder, CliError> {
    let Some(metadata) = metadata else {
        return Ok(builder);
    };

    let execution_tag = Tag::parse([OMPK_EXECUTION_TAG, OMPK_EXECUTION_CONTRACT_VERSION])
        .map_err(|e| CliError::Other(format!("failed to encode OMPK execution tag: {e}")))?;
    let mut builder = builder.tag(execution_tag);
    if let Some(cwd) = &metadata.cwd {
        let cwd_tag = Tag::parse([OMPK_CWD_TAG, cwd.as_str()])
            .map_err(|e| CliError::Other(format!("failed to encode OMPK cwd tag: {e}")))?;
        builder = builder.tag(cwd_tag);
    }
    Ok(builder)
}
...
pub async fn cmd_send_message(
    ...
    // Validate execution metadata before any relay lookup, upload, or other
    // side effect. Filesystem authorization remains with the ACP/OMPK boundary.
    let ompk_execution = validate_ompk_execution_metadata(p.ompk_execution, p.ompk_cwd.as_deref())?;
    ...
    let builder = with_ompk_execution_metadata(builder, ompk_execution.as_ref())?;
...
mod tests {
    use super::{
        ...
        resolve_names_to_pubkeys, validate_ompk_execution_metadata, with_ompk_execution_metadata,
        OMPK_CWD_TAG, OMPK_EXECUTION_CONTRACT_VERSION, OMPK_EXECUTION_TAG,
    };
    use buzz_sdk::mentions::{
    ...
    #[cfg(windows)]
    const ABSOLUTE_WORKSPACE: &str = r"C:\workspace\repo with spaces";
    #[cfg(not(windows))]
    const ABSOLUTE_WORKSPACE: &str = "/workspace/repo with spaces";

    #[test]
    fn ordinary_send_has_no_ompk_execution_metadata() {
        use nostr::{EventBuilder, Keys, Kind, Tag};

        let metadata = validate_ompk_execution_metadata(false, None).unwrap();
        assert!(metadata.is_none());

        let event = with_ompk_execution_metadata(
            EventBuilder::new(Kind::Custom(9), "ordinary")
                .tag(Tag::parse(["h", "channel-id"]).unwrap()),
            metadata.as_ref(),
        )
        .unwrap()
        ...
        let tags = event
            ...
            .collect::<Vec<_>>();
        assert!(tags.contains(&vec!["h".into(), "channel-id".into()]));
        assert!(!tags.iter().any(|tag| {
            matches!(
                tag.first().map(String::as_str),
                Some(OMPK_EXECUTION_TAG | OMPK_CWD_TAG)
            )
        }));
    }

    #[test]
    fn execution_metadata_is_encoded_as_exact_signed_tags() {
        use nostr::{EventBuilder, Keys, Kind};

        let metadata = validate_ompk_execution_metadata(true, Some(ABSOLUTE_WORKSPACE)).unwrap();
        let event = with_ompk_execution_metadata(
            EventBuilder::new(Kind::Custom(9), "bounded task"),
            metadata.as_ref(),
        )
        .unwrap()
        .sign_with_keys(&Keys::generate())
        ...
        assert!(tags.contains(&vec![
            OMPK_EXECUTION_TAG.into(),
            OMPK_EXECUTION_CONTRACT_VERSION.into(),
        ]));
        assert!(tags.contains(&vec![OMPK_CWD_TAG.into(), ABSOLUTE_WORKSPACE.into()]));
        assert!(
            event.verify().is_ok(),
            "metadata must be covered by the signature"
        );
    }

    #[test]
    fn ompk_cwd_validation_rejects_unbound_or_unsafe_values_without_echoing_them() {
        let secret = "relative/TOKEN_super_secret";
        let unbound = validate_ompk_execution_metadata(false, Some(ABSOLUTE_WORKSPACE))
            .unwrap_err()
            .to_string();
        assert!(unbound.contains("requires --ompk-execution"));

        for invalid in ["", secret, "absolute\nnewline"] {
            let error = validate_ompk_execution_metadata(true, Some(invalid))
                .unwrap_err()
                .to_string();
            if !invalid.is_empty() {
                assert!(!error.contains(invalid));
            }
            assert!(!error.contains("TOKEN_super_secret"));
        }

        let oversized = format!("{}{}", ABSOLUTE_WORKSPACE, "x".repeat(4096));
        let error = validate_ompk_execution_metadata(true, Some(&oversized))
            .unwrap_err()
            .to_string();

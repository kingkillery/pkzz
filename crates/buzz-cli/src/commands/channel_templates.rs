...
fn load_templates(path: &Path) -> Result<Vec<ChannelTemplateRecord>, CliError> {
    if !path.exists() {
        return Err(CliError::NotFound(format!(
            "no channel templates store found at {} (create a template in Pkzz Desktop first, \
...
mod tests {
    ...
    fn find_template_matches_case_insensitive() {
        let f = write_store(r#"[{"id":"t1","name":"Pkzz Team","createdAt":"x","updatedAt":"x"}]"#);
        let t = find_template(f.path(), "buzz team").expect("found");
        assert_eq!(t.name, "Pkzz Team");
    ...
    fn load_templates_parses_full_roster() {
        let f = write_store(
            r##"[{
                "id":"t1","name":"Pkzz Team","channel_type":"forum","visibility":"private",
                "canvas_template":"# {channel.name}",
                "agents":{"personas":[{"personaId":"builtin:fizz"}],"teams":[{"teamId":"team-1"}]},
                "created_at":"x","updated_at":"x"
            }]"##,
        );
        let t = find_template(f.path(), "Pkzz Team").expect("found");

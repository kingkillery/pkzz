...
async fn tokio_main() -> Result<()> {
    ...
    let ctx = Arc::new(PromptContext {
        ...
        cwd: std::env::current_dir()
            ...
            .to_string_lossy()
            .to_string(),
        ompk_execution: config.ompk_execution.clone(),
...
fn try_native_steer(
    ...
    // `queue::format_prompt` uses internally for `[Pkzz event: …]`
...
fn handle_prompt_result(
    ...
    if let Some(batch) = result.batch.take() {
        ...
        if !removed_channels.contains(&batch.channel_id) {
            ...
            } else if let PromptOutcome::Error(acp::AcpError::InvalidRequest(error)) =
                &result.outcome
            {
                // Signed control metadata is deterministic. Retrying the same
                // event cannot make an invalid or unauthorized placement valid,
                // so fail once and return the notice to its originating thread.
                tracing::warn!(
                    channel_id = %batch.channel_id,
                    events = batch.events.len(),
                    "dead-lettering batch immediately — invalid execution request"
                );
...
mod build_mcp_servers_tests {
    ...
    fn test_config() -> Config {
        Config {
            ...
            lazy_pool: false,
            ompk_execution: crate::ompk_execution::OmpkExecutionPolicy::new(true, vec![])
...
mod observer_payload_trim_tests {
    ...
    fn test_multi_block_prompt_retains_every_section_header_after_elision() {
        ...
        // {text: "[Agent Memory — core]…"}, … {text: "[Pkzz event: …]…<huge>"}].
        ...
        // [Pkzz event] header fell into the elided middle and the count collapsed
        // to 1).
        let sections = [
        ...
        let event_block = texts
            .iter()
            .find(|t| t.starts_with("[Pkzz event: @mention]"))
            .unwrap();
        assert!(

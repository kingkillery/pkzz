...
- **Host-final capability exception:** Only when the host injects a `[Host Final Reply]` instruction for this session, your ordinary final assistant text is delivered by the host. Do not call `buzz messages send` for that same answer. The standing CLI rule above remains in force for every other session and for intentional additional, proactive, or different-destination messages.

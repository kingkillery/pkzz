    ...
    fn nudge_body(&self) -> String {
        let prose = if self.requirements.is_empty() {
            ...
            let footer = if has_doctor_requirement {
                ...
                // Mixed: some Pkzz-managed fields, some external config.
                "Open Edit Agent in the Pkzz app for the Pkzz-managed fields; fix the external CLI config files manually and restart the agent.".to_string()
            } else {
                // All Pkzz-managed — original footer unchanged.
                "Open Edit Agent in the Pkzz app to set these.".to_string()
            };
...
mod tests {
    ...
    fn setup_payload_deserializes_git_bash_requirement() {
        let payload: SetupPayload = serde_json::from_str(
            r#"{"agent_name":"Pkzz Agent","agent_pubkey":"test","requirements":[{"surface":"git_bash"}]}"#,
        )
        .unwrap();
        assert!(matches!(
    ...
    fn nudge_body_git_bash_copy_points_to_agent_runtimes() {
        let payload = SetupPayload {
            agent_name: "Pkzz Agent".to_string(),
            agent_pubkey: "test".to_string(),
    ...
    fn nudge_body_mixed_requirements_uses_split_footer() {
        // Mixed list: one Pkzz-managed env key + one external config invalid.
    ...
    fn nudge_body_all_buzz_managed_retains_original_footer() {
        // Pure Pkzz-managed requirements → original "Open Edit Agent" footer unchanged.
        let payload = SetupPayload {
            agent_name: "Fizz".to_string(),
            agent_pubkey: "test".to_string(),
        ...
        let body = payload.nudge_body();
        assert!(
            body.contains("Open Edit Agent in the Pkzz app to set these."),

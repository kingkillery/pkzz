...
pub struct SessionState {
    ...
    /// channel_id → cwd used for `session/new`.
...
pub struct PromptContext {
    ...
    pub cwd: String,
    /// Additive OMPK request adapter. The pool consumes only its validated cwd
    /// selection; execution remains owned by the ACP backend.
    pub ompk_execution: OmpkExecutionPolicy,
...
pub async fn run_prompt_task(
    ...
    // Parse the optional backend-specific contract before resolving a session.
    // The adapter validates signed structural tags and returns only a bounded
    // execution identity plus cwd; conversational content never becomes an
    // execution parameter.
    let execution_request = match batch.as_ref() {
        Some(batch) => match ctx.ompk_execution.parse_events(
            batch.events.iter().map(|batch_event| &batch_event.event),
            &agent.agent_name,
        ) {
            Ok(request) => request,
            Err(error) => {
                ...
                send_prompt_result(
                    ...
                    agent,
                    source,
                    PromptOutcome::Error(AcpError::InvalidRequest(error.to_string())),
                    Some(batch.clone()),
                );
                return;
            }
    ...
    let session_cwd = execution_request
        .as_ref()
        .and_then(|request| request.cwd.as_ref())
    ...
    if let Some(request) = execution_request.as_ref() {
        agent.acp.observe(
            ...
            serde_json::json!({
                ...
                "placementKind": if request.cwd.is_some() { "cwd" } else { "default" },
            }),
        );
    }

    // A session's cwd is immutable in ACP. Rotate whenever the desired value
    // changes, including explicit-cwd -> ordinary/default turns, so placement
    // never becomes contagious across requests.
    if let PromptSource::Channel(channel_id) = &source {
        if agent.state.rotate_if_cwd_changed(channel_id, &session_cwd) {
            tracing::info!(
                target: "pool::session",
                channel = %channel_id,
                "rotating ACP session because the selected cwd changed"
            );
        }
    }
...
mod tests {
    ...
    fn session_is_reused_only_for_the_same_cwd() {
        ...
        state.sessions.insert(channel, "session-a".into());
        state.session_cwds.insert(channel, "C:\\workspace-a".into());

        assert!(!state.rotate_if_cwd_changed(&channel, "C:\\workspace-a"));
        assert_eq!(
    ...
    #[test]
    fn explicit_cwd_cannot_leak_into_a_later_default_turn() {
        let channel = Uuid::new_v4();
        let mut state = SessionState::default();
        state.sessions.insert(channel, "placed-session".into());
        state
            .session_cwds
            .insert(channel, "C:\\explicit-workspace".into());

        assert!(state.rotate_if_cwd_changed(&channel, "C:\\default-workspace"));
    ...
    fn make_prompt_context_impl(
        ...
        PromptContext {
            ...
            base_prompt: None,
            cwd: ".".to_string(),
            ompk_execution: OmpkExecutionPolicy::new(true, vec![])
                .expect("empty OMPK policy is valid"),
            rest_client: RestClient {

...
A Pkzz **community** is the tenant-visible workspace selected by the request host.
...
Pkzz is a Rust monorepo, licensed Apache 2.0 under Block, Inc.
...
Pkzz uses Nostr NIP-01 on the wire. Every action is a JSON event with six fields:
...
| 40000–49999 | Pkzz custom kinds |

### Pkzz Custom Kinds (selected)
...
`buzz-core` defines each event kind as a `pub const u32` and exports the full registry as `ALL_KINDS: &[u32]` (127 kinds at the time of writing); `crates/buzz-core/src/kind.rs` is the source of truth for the current list. Kinds are `u32` (NIP-01 specifies unsigned integer; `u32` covers the full range). Pkzz uses both standard Nostr kinds (e.g., kind 7 for reactions) and custom ranges (40000+).
...
Standalone binary that bridges Pkzz relay events to AI agents via the [Agent Communication Protocol](https://agentclientprotocol.com/) (ACP).

**Architecture:**

```
Pkzz Relay ──WS──→ buzz-acp ──stdio (ACP/JSON-RPC)──→ Agent (goose/codex/claude)
```
